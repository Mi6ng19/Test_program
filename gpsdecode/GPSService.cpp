...
#include <gpsdecode/GPSService.h>
#include <gpsdecode/NumberConversion.h>
...
void GPSService::attachToParser(NMEAParser& _parser){
	...
	*/
    ...
    //北斗
    _parser.setSentenceHandler("GBGGA", [this](const NMEASentence& nmea){
        this->read_GPGGA(nmea);
    });
    _parser.setSentenceHandler("GBRMC", [this](const NMEASentence& nmea){
        this->read_GPRMC(nmea);
    });
    _parser.setSentenceHandler("GBVTG", [this](const NMEASentence& nmea){
        this->read_GPVTG(nmea);
    });
    _parser.setSentenceHandler("GBGSA", [this](const NMEASentence& nmea){
        this->read_GBGSA(nmea);
    });
    _parser.setSentenceHandler("GBGSV", [this](const NMEASentence& nmea){
        this->read_GBGSV(nmea);
    });
...
QString GPSService::getSBARSPrn(uint32_t src)
{
...
void GPSService::read_GPGGA(const NMEASentence& nmea){
	...
	*/
	try
	{
        ...
        if (nmea.parameters[7].size() > 0){
            this->fix.accuracy = parseDouble(nmea.parameters[7])*5;
        }
...
void GPSService::read_GPGSA(const NMEASentence& nmea){
	...
	*/
	...
	try
	{
        ...
        if (this->fix.almanac.satellitesT.size() >= this->fix.trackingSatellites){
            this->fix.almanac.satellitesT.clear();
        }
//        this->fix.almanac.satellitesT.clear();
        II ii;
        for (int j=2; j<this->fix.trackingSatellites+2; j++){
  //          printf("=== %d\n", (uint32_t)parseInt(nmea.parameters[j]));
            ii.id = getSBARSPrn((uint32_t)parseInt(nmea.parameters[j]));
            ii.idx = 0;
            this->fix.almanac.satellitesT.push_back(ii);
        }
...
//        if (currentPage == 1){
//            this->fix.almanac.satellitesGP.clear();
//        }

		//if this is the first page, then reset the almanac

        this->fix.almanac.lastPageGP = currentPage;
        this->fix.almanac.totalPagesGP = totalPages;
		...
		for (int i = 0; i < entriesInPage; i++){
            ...
            sat.prn = getSBARSPrn((uint32_t)parseInt(nmea.parameters[prop]));
            ...
            this->fix.almanac.satellites.push_back(sat);
            this->fix.visibleSatellites++;
        }
        if (this->fix.almanac.satellites.size()>31 || this->fix.almanac.satellitesT.size()>31) this->fix.almanac.clear();
...
//            sat.idx = 0;
//            //cout << "ADDING SATELLITE ::" << sat.toString() << endl;
//            this->fix.almanac.satellitesGP.push_back(sat);
//        }
...
void GPSService::read_GPRMC(const NMEASentence& nmea){
	...
	*/
	...
	try
	{
		...
		// TIMESTAMP
		this->fix.timestamp.setTime(parseDouble(nmea.parameters[0]));
        this->fix.location = *nmea.parameters[1].c_str();
...
void GPSService::read_GPVTG(const NMEASentence& nmea){
	...
	*/
	...
	try
	{
        ...
        this->fix.direct = -1;
        this->fix.direct = parseDouble(nmea.parameters[0]);
...
void GPSService::read_GNGSA(const NMEASentence &nmea)
{
    /*  -- EXAMPLE --
    $GPGSA,A,3,04,05,,09,12,,,24,,,,,2.5,1.3,2.1*39
...
//        this->fix.almanac.satellitesT.clear();
        ...
        for (int j=2; j<this->fix.trackingSatellites+2; j++){
           // printf("=== %d\n", (uint32_t)parseInt(nmea.parameters[j]));
            if (fix.almanac.gsaPage && parseInt(nmea.parameters[j])<50){
                ii.id = QString("b")+QString::number((uint32_t)parseInt(nmea.parameters[j]));
                if (ii.id == "b0") continue;
            } else {
                ii.id = getSBARSPrn((uint32_t)parseInt(nmea.parameters[j]));
                ii.idx = 0;
                if (ii.id == "0") continue;
        ...
        fix.almanac.gsaPage++;
//        for (int i=0; i<fix.almanac.satellitesT.size(); i++){
//            printf("=== %s\n", fix.almanac.satellitesT.at(i).id.toUtf8().data());
//        }
...
//读取北斗
void GPSService::read_GBGSV(const NMEASentence &nmea)
{
    /*  -- EXAMPLE --
    $GPGSV,2,1,08,01,40,083,46,02,17,308,41,12,07,344,39,14,22,228,45*75
...
//        }
        ...
        for (int i = 0; i < entriesInPage; i++){
            ...
            // PRN, ELEVATION, AZIMUTH, SNR
            sat.prn = QString("b")+QString::number((uint32_t)parseInt(nmea.parameters[prop]));
            sat.elevation = (uint32_t)parseInt(nmea.parameters[prop + 1]);
            sat.azimuth = (uint32_t)parseInt(nmea.parameters[prop + 2]);
            sat.snr = (uint32_t)parseInt(nmea.parameters[prop + 3]);
            sat.idx = 0;
...
void GPSService::read_GBGSA(const NMEASentence &nmea)
{
    /*  -- EXAMPLE --
    $GPGSA,A,3,04,05,,09,12,,,24,,,,,2.5,1.3,2.1*39
